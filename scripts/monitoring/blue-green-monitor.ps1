...
while ($true) {
    ...
    try {
        ...
        aws ecs describe-services --cluster "$ProjectName-cluster" --services "$ProjectName-api-service" --query 'services[0].deployments[*].{Status:status,TaskDef:taskDefinition,Running:runningCount,Desired:desiredCount,UpdatedAt:updatedAt}' --output table
        
        # Blue Target Group Health
        Write-Host "`n--- Blue TG Health ---" -ForegroundColor Blue
        $blueTgArn = aws elbv2 describe-target-groups --names "$ProjectName-blue-tg" --query 'TargetGroups[0].TargetGroupArn' --output text
        ...
        $greenTgArn = aws elbv2 describe-target-groups --names "$ProjectName-green-tg" --query 'TargetGroups[0].TargetGroupArn' --output text
        ...
        $albArn = aws elbv2 describe-load-balancers --names "$ProjectName-alb" --query 'LoadBalancers[0].LoadBalancerArn' --output text
        ...
        aws ecs list-tasks --cluster "$ProjectName-cluster" --service-name "$ProjectName-api-service" --query 'taskArns[*]' --output table
        
    } catch {
        Write-Host "Error occurred: $_" -ForegroundColor Red
    }
    
    Write-Host "`n--- Next update in $MonitorInterval seconds (Ctrl+C to stop) ---" -ForegroundColor Gray
    $iteration++
