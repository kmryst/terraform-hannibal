...
| --- | --- | --- | --- | --- |
| `HannibalDeveloperRole-Dev` | 日常開発・アプリ運用 | `hannibal` IAM User | ECS exec / ログ確認 / ECR 操作 / devアプリ環境の確認・手動操作用。IAM write / PassRole は `nestjs-hannibal-3-*` のアプリ用 Role / Policy に寄せ、`Hannibal*` Role / Policy・GitHub OIDC・foundation state・CloudTrail / Athena / Budgets は `HannibalFoundationRole-Dev` へ分離する。#164 の候補検証が終わるまでは既存権限を維持する | `terraform/foundation`。厳密運用で変更 |
| `HannibalDeveloperRole-Dev-candidate` | Developer Role 最小権限化検証 | `hannibal` IAM User | `HannibalDeveloperPolicy-Dev-candidate` と `HannibalDeveloperBoundary-Dev-candidate` を付与した検証用 Role。既存 Developer Role を壊さず、ECS exec / ログ確認 / ECR push / Secrets 参照 / dev state lock を確認する。検証後に本体 Role へ反映し、この candidate は削除する | `terraform/foundation`。#164 の段階移行用 |
| `HannibalFoundationRole-Dev` | `terraform/foundation` の手動 apply | `hannibal` IAM User | IAM / OIDC / Permission Boundary / CloudTrail / Athena / GuardDuty / Budgets / foundation state / S3 lockfile 操作用。DynamoDB lock は移行期間中のみ併用。専用 Boundary `HannibalFoundationBoundary-Dev` で foundation 対象サービスに最大権限を制限。Foundation Role は自身の Boundary を更新しない | `terraform/foundation`。厳密運用で変更 |
...
│  └── アタッチポリシー: HannibalDeveloperPolicy-Dev（日常開発・アプリ運用。既存運用中）
└── 検証用ロール: HannibalDeveloperRole-Dev-candidate
   ├── Permission Boundary: HannibalDeveloperBoundary-Dev-candidate
   └── アタッチポリシー: HannibalDeveloperPolicy-Dev-candidate

🧱 hannibal (foundation手動apply)
└── 使用可能ロール: HannibalFoundationRole-Dev
   ├── Permission Boundary: HannibalFoundationBoundary-Dev
   └── アタッチポリシー: HannibalFoundationPolicy-Dev
...
aws sts assume-role --role-arn arn:aws:iam::258632448142:role/HannibalDeveloperRole-Dev-candidate --role-session-name dev-candidate-session

# terraform/foundation 手動 apply (hannibal)
# HannibalDeveloperRole-Dev では実行しない
aws sts assume-role --role-arn arn:aws:iam::258632448142:role/HannibalFoundationRole-Dev --role-session-name foundation-apply-session
...
`HannibalDeveloperRole-Dev` に残す権限は、日常開発・アプリ運用に必要なものに寄せる。具体的には、ECR image 操作、ECS exec / service / task 確認、CloudWatch Logs 確認、devアプリ環境の Terraform refresh / plan / 手動操作、および `nestjs-hannibal-3-*` prefix のアプリ用 IAM Role / Policy 管理を対象にする。

`HannibalDeveloperRole-Dev` から外す権限は、`Hannibal*` Role / Policy の作成・削除・ポリシー更新、GitHub OIDC Provider、Permission Boundary、foundation state、CloudTrail / Athena / Budgets などの foundation apply 用権限とする。これらは `HannibalFoundationRole-Dev` を assume して厳密運用で扱う。

#164 では既存 `HannibalDeveloperRole-Dev` を直接縮小せず、`HannibalDeveloperRole-Dev-candidate` を先に作る。candidate で ECS exec / CloudWatch Logs / ECR push / Secrets 参照 / `terraform/environments/dev` の lock あり plan を確認し、AccessDenied があれば candidate policy を修正する。検証後の PR で `HannibalDeveloperPolicy-Dev` と Developer Role の Boundary へ反映し、candidate は削除する。
