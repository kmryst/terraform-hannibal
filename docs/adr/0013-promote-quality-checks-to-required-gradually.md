...
この ADR は、Issue #226 / PR #227 と Issue #228 で実践した判断を事後的に明文化したものである。現時点での required status checks は `TFLint` と `Gitleaks Secret Scan` であり、`Trivy Config Scan` は既存 finding の分類と accepted risk / ignore の整理が完了するまで review signal として扱う。
...
- 新しい PR check を追加する場合は、(1) 検出時に `job fail` させるか review signal に留めるか、(2) branch protection の required status check に含めるか（含める場合は観察期間後に判断するか）を、それぞれ明示する
