...
`terraform/environments/dev/` 配下のアプリ全体インフラは、通常は `terraform destroy` 済みで停止しておき、動作確認や動的デモが必要なときだけ `deploy.yml`（`deployment_mode: provisioning`）で起動する「オンデマンド起動 / 通常 destroy 運用」を採用する。
...
このプロジェクトは本番サービスではなく、必要なときに起動し使い終わったら破棄する on-demand / ephemeral environment（一時環境）である。継続的にユーザートラフィックを受けるものではなく、稼働率は低い。一方で、ECS Fargate / RDS / ALB / CodeDeploy Blue-Green といった動的なインフラを、IaC で再現可能な形で構築・破棄できること自体に価値がある。

一方で、これらの一時リソースを常時起動しておくと、稼働していない時間帯にも継続的に課金が発生する。この環境は稼働率が低く、待機している時間がほとんどになる。
...
- 短所: 構成スペックからの試算で月額 $30-50 程度（NAT Gateway・ALB・RDS・Fargate が主因）が継続的に発生する。停止運用の $5 以下と比べ、稼働率の低い ephemeral environment には割高
...
- 短所: ECS / RDS / Blue-Green デプロイなどの動的インフラを動かせず、IaC の再現性・運用設計を実証できなくなる
...
待機時 $5 程度と起動時 $30-50 程度（試算）には大きな差があり、稼働率の低い ephemeral environment では常時起動のコストは割に合わない。

加えて、destroy 済み状態から `deploy.yml` で毎回再構築できることは、IaC が壊れず冪等に再現できることの実証になる。完全削除では動的インフラを実証できないため、一時リソースは IaC として保持したうえで、通常は destroy しておく本運用が、コストと再現性の実証のバランスとして最も適している。
