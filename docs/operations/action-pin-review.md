...
追記: 2026-06-24 に Issue #366 として、次回 github-actions Dependabot PR での Tier B SHA pin 追従更新を確認した。
...
|---|---|---|
| `actions/checkout` | `v7.0.0` | `sync-labels.yml`, `security-scan.yml`, `destroy.yml`, `deploy.yml`, `pr-check.yml` |
...
| `aws-actions/amazon-ecr-login` | `d539f0932e70871a027e9d5a9d8fc38589180a64 # v2.1.6` | `deploy.yml` |
...
| `aws-actions/amazon-ecr-login` (v2.1.6, annotated tag) | `d539f0932e70871a027e9d5a9d8fc38589180a64` | `d539f0932e70871a027e9d5a9d8fc38589180a64` | ✅ |
...
## 7. Dependabot PR #417 での追従更新確認（Issue #366）

2026-06-24 に merge された [PR #417](https://github.com/kmryst/terraform-hannibal/pull/417) で、github-actions group の Dependabot version update を確認した。

対象更新:

- Tier A: `actions/checkout` が `v6.0.3` から `v7.0.0` へ更新された
- Tier B: `aws-actions/amazon-ecr-login` が `fa648b43de3d4d023bcb3f89ed6940096949c419 # v2.1.5` から `d539f0932e70871a027e9d5a9d8fc38589180a64 # v2.1.6` へ更新された

Tier B の確認結果:

```text
$ git ls-remote https://github.com/aws-actions/amazon-ecr-login.git refs/tags/v2.1.6 refs/tags/v2.1.6^{}
b040164c4934333d597f3f9c67502ff28f814e9c refs/tags/v2.1.6
d539f0932e70871a027e9d5a9d8fc38589180a64 refs/tags/v2.1.6^{}
```

`v2.1.6` は annotated tag であり、dereference 後の commit は `d539f0932e70871a027e9d5a9d8fc38589180a64`。PR #417 が更新した `uses:` の SHA と一致した。

PR #417 の確認では、Dependabot が Tier B action の `@<sha>` と同一行の `# vX.Y.Z` コメントを同時に追従更新することを確認できた。補完策の追加は不要と判断する。

追加確認:

- release notes / changelog: `aws-actions/amazon-ecr-login` `v2.1.6` は main module check の修正と依存更新が中心であり、workflow input / permissions / secrets / OIDC / artifact 入出力の追加変更は確認されなかった
- advisory: `GET /advisories?ecosystem=actions&affects=aws-actions/amazon-ecr-login` は `[]` を返し、該当 advisory はなかった
- CI: PR #417 は merge 済み。確認時点の `gh pr checks 417` では Backend / Frontend / Docker / Terraform / TFLint / Gitleaks / Trivy の各 check は pass、Dependabot bot のため `PR Policy Check` は skip だった。`Commitlint` は fail だったが、Tier B SHA / コメント追従確認そのものには影響しないため、本 Issue では追加対応しない

## 8. 次回以降の確認事項

- 今後 Tier B action の Dependabot version update PR が出た場合も、本ドキュメント section 6 のレビュー手順を適用する。

## 関連

- [Issue #356](https://github.com/kmryst/terraform-hannibal/issues/356)
- [Issue #366](https://github.com/kmryst/terraform-hannibal/issues/366)
