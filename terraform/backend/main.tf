...
resource "aws_ecs_task_definition" "api" {                            # APIサーバ用のコンテナなので
  ...
  # Professional設計: deploy.ymlの--cli-input-json処理でnull値を安全に除去するため、明示的にnullを設定しない
  ...
  container_definitions = jsonencode([
    {
      ...
      environment = [                                            # コンテナ内部のアプリに渡す環境変数
        ...
        { name = "CLIENT_URL", value = coalesce(var.client_url_for_cors, "https://hamilcar-hannibal.click") }, # CORS設定用のフロントエンドURL
        { name = "DATABASE_URL", value = "postgresql://${var.db_username}:${var.db_password}@${aws_db_instance.postgres.endpoint}/${var.db_name}?sslmode=disable&connect_timeout=30&application_name=nestjs-hannibal-3" },
        { name = "DB_HOST", value = aws_db_instance.postgres.endpoint },
        { name = "DB_PORT", value = "5432" },
        { name = "DB_NAME", value = var.db_name },
        { name = "DB_USERNAME", value = var.db_username },
        { name = "DB_PASSWORD", value = var.db_password },
        # Professional: ハンニバルデータ初期化用
        { name = "SEED_INITIAL_DATA", value = "true" },
...
resource "aws_lb_target_group" "blue" {
  ...
  health_check {
    ...
    protocol            = "HTTP"
    port                = "traffic-port"
    healthy_threshold   = 2  # Professional: 最小推奨
    unhealthy_threshold = 2  # Professional高速: 短縮
    timeout             = 5  # Professional高速: 短縮
    interval            = 15 # Professional高速: 短縮
    matcher             = "200-399"
  }
  tags = {
...
resource "aws_ecs_service" "green" {
    ...
    cluster                           = aws_ecs_cluster.main.id
    task_definition                   = aws_ecs_task_definition.api.arn
    desired_count                     = 0  # AWS Professional: CodeDeploy競合回避のため初期は0
    launch_type                       = "FARGATE"
    health_check_grace_period_seconds = 600
...
# --- RDS Parameter Group (Professional最短時間最適化) ---
