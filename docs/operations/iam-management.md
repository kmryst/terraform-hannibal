...
| `HannibalCICDRole-Dev` | main の deploy / destroy | GitHub Actions OIDC `repo:kmryst/terraform-hannibal:ref:refs/heads/main` | deploy / destroy 用。PR plan には使わない。ポリシーは 3 分割（compute/storage/deploy）で Terraform 管理（#166）。Boundary `HannibalCICDBoundary`に`fis:*`をceilingとして追加済み（#446）。`HannibalCICDPolicy-Dev-deploy`にFIS実験テンプレート管理権限（`experiment-template/*`に限定）と`HannibalFISRole-Dev`への`iam:PassRole`（`PassedToService=fis.amazonaws.com`条件）を追加済み（#454。deploy.ymlがterraform/serviceの`aws_fis_experiment_template`をapplyするために必要と判明） | `terraform/foundation`。厳密運用で変更 |
