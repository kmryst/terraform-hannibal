...
- Commitlint / Sync Labels / PR Policy Check は `idp-golden-path` の reusable workflow を `@v1` で消費する薄い caller workflow に移行済み（Issue #495 / #498 / #496）。Gitleaks はこのリポジトリ内のローカル workflow のまま。
