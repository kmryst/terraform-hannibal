...
このプロジェクトは本番サービスではなく、デモ / ショーケース用途の環境であり、必要時だけ起動して通常は destroy 済みで停止しておく運用を前提にしている（[0008](./0008-on-demand-startup-and-routine-destroy-operation.md)）。
...
現時点では、常時公開ではない dev / デモ環境に対して GuardDuty の固定的な運用コストを払う優先度は低い。
