...
- DynamoDB-based locking は現行構成で使用しない。移行履歴と旧 `terraform-state-lock` table の cleanup は [ADR 0003](../adr/0003-migrate-terraform-state-locking-to-s3-lockfile.md) と Issue #189 を参照する。
...
| IAM Policy | `HannibalCICDPolicy-Dev-compute` | CI/CD Role に attach 中の compute 系ポリシー |
| IAM Policy | `HannibalCICDPolicy-Dev-storage` | CI/CD Role に attach 中の storage 系ポリシー |
| IAM Policy | `HannibalCICDPolicy-Dev-deploy` | CI/CD Role に attach 中の deploy 系ポリシー |
| IAM Policy | `HannibalFoundationPolicy-Dev` | foundation apply 用ポリシー |
| IAM Policy | `HannibalFoundationStatePolicy-Dev` | foundation backend state / lock 用ポリシー |
| IAM Policy | `HannibalFoundationServicesPolicy-Dev` | CloudTrail / Athena / Glue / GuardDuty / Budgets などの foundation 管理サービス用ポリシー |
| IAM Permission Boundary | `HannibalCICDBoundary` | CI/CD Role の権限上限。`terraform/foundation` で管理 |
| IAM Permission Boundary | `HannibalECSBoundary` | ECS タスク実行 Role の権限上限。`terraform/foundation` で管理 |
...
- `nestjs-hannibal-3-cloudtrail-logs` / `nestjs-hannibal-3-athena-results` は存在する S3 バケットであり、現在は `terraform/foundation` で管理する。
- 古い検討用の `HannibalCICDPolicy-Dev-Minimal` / `HannibalCICDPolicy-Dev-updated` JSON は Terraform から参照されていないため削除済み。
...
- [docs/operations/terraform-runbook.md](./terraform-runbook.md) — Terraform init / plan / apply / state lock / import / drift 確認
- [docs/operations/rollback-plan.md](./rollback-plan.md) — Terraform rollback / state 復元手順
