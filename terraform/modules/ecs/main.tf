...
resource "aws_ecs_task_definition" "api" {                  # APIサーバ用のコンテナなので
  ...
  # ECS上で管理される "arn:aws:ecs:ap-northeast-1:<account-id>:task-definition/nestjs-hannibal-3-api-task:<revision>"
  ...
  container_definitions = jsonencode([
    {
      ...
      environment = [                                            # コンテナ内部のアプリに渡す環境変数
        ...
        # DB TLS parameters are not secrets; keep as plain env vars.
        { name = "DB_NAME", value = var.db_name },
        { name = "DB_SSLMODE", value = "require" },
        { name = "DB_SSLROOTCERT", value = "/opt/rds-ca-2019-root.pem" },

        # RDS 管理シークレットに host/port が含まれないため、エンドポイント由来を平文 env で渡す（非機密）
        { name = "DB_HOST", value = local.rds_host },
        { name = "DB_PORT", value = local.rds_port }
      ]
      secrets = [
        # RDS が Secrets Manager に保存するのは username / password のみ
