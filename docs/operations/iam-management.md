...
| --- | --- | --- | --- | --- |
| `HannibalDeveloperRole-Dev` | 開発者の手元作業 | `hannibal` IAM User | dev作業用の広い開発権限。foundation apply 用権限は `HannibalFoundationRole-Dev` へ分離し、Developer Role 側の縮小は #180 で扱う | `terraform/foundation`。厳密運用で変更 |
| `HannibalFoundationRole-Dev` | `terraform/foundation` の手動 apply | `hannibal` IAM User | IAM / OIDC / Permission Boundary / CloudTrail / Athena / GuardDuty / Budgets / DynamoDB lock / foundation state 操作用。専用 Boundary `HannibalFoundationBoundary-Dev` で foundation 対象サービスに最大権限を制限 | `terraform/foundation`。厳密運用で変更 |
