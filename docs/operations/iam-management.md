...
| `HannibalPRPlanRole-Dev` | PR の `terraform plan` | GitHub Actions OIDC `repo:kmryst/terraform-hannibal:pull_request` | read-only plan。apply / destroy / write 系権限なし | #127 (PR #140) で実装済み。詳細は [pr-terraform-plan-role-design.md](./pr-terraform-plan-role-design.md) |
