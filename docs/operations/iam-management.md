...
| --- | --- | --- | --- | --- |
| `HannibalDeveloperRole-Dev` | 日常開発・アプリ運用 | `hannibal` IAM User | ECS exec / ログ確認 / ECR push / Secrets 参照 / dev Terraform plan 専用。write/exec は action 列挙・Resource スコープ済み。read 系（`Get*`/`Describe*`/`List*`）はプレフィックス wildcard の意図的設計（#164）。Boundary `HannibalDeveloperBoundary-Dev` で上限を設定。`Hannibal*` IAM・OIDC・foundation state・CloudTrail / Athena / Budgets は `HannibalFoundationRole-Dev` で扱う | `terraform/foundation`。厳密運用で変更 |
| `HannibalFoundationRole-Dev` | `terraform/foundation` の手動 apply | `hannibal` IAM User | IAM / OIDC / Permission Boundary / CloudTrail / Athena / Glue Data Catalog / GuardDuty / Budgets / foundation state / S3 lockfile / 監査・分析用 S3 bucket 設定 操作用。ポリシーは 3 分割（core/state/services）で Terraform 管理し、managed policy の文字数上限に余裕を持たせる。専用 Boundary `HannibalFoundationBoundary-Dev` で foundation 対象サービスに最大権限を制限。Foundation Role は自身の Boundary を更新しない | `terraform/foundation`。厳密運用で変更 |
...
| `HannibalCICDRole-Dev` | main の deploy / destroy | GitHub Actions OIDC `repo:kmryst/terraform-hannibal:ref:refs/heads/main` | deploy / destroy 用。PR plan には使わない。ポリシーは 4 分割（compute/storage/deploy/synthetics）で Terraform 管理（#166、#469）。Boundary `HannibalCICDBoundary`に`fis:*`をceilingとして追加済み（#446）。`HannibalCICDPolicy-Dev-deploy`にFIS実験テンプレート管理権限（`experiment-template/*`に限定）と`HannibalFISRole-Dev`への`iam:PassRole`（`PassedToService=fis.amazonaws.com`条件）を追加済み（#454。deploy.ymlがterraform/serviceの`aws_fis_experiment_template`をapplyするために必要と判明）。`HannibalCICDPolicy-Dev-synthetics` で CloudWatch Synthetics canary / cwsyn Lambda / canary log / X-Ray read / canary実行roleへの`iam:PassRole`を許可し、storage policyではSynthetics artifact bucketとproject secretの包括的な操作を許可する（#469）。canary実行roleのinline policy作成に必要なproject role向け`iam:PutRolePolicy`等はBoundary側のceilingにも含める（#471） | `terraform/foundation`。厳密運用で変更 |
...
| `HannibalFISRole-Dev` | Game Day演習（AWS FISによるECSタスク強制停止）の実験実行 | `fis.amazonaws.com`（`aws:SourceAccount`と`aws:SourceArn`(experiment ARN)で確認済み） | `ecs:StopTask`を`nestjs-hannibal-3-cluster`のtaskのみに限定。`ecs:DescribeTasks`/`ecs:ListTasks`/`ecs:DescribeClusters`はresource-level権限非対応のためResource `*`。専用Boundary `HannibalFISBoundary-Dev`で同一権限を上限として設定。設計判断は[ADR-0027](../adr/0027-fis-iam-permission-boundary-for-game-day.md)参照 | `terraform/foundation`。厳密運用で変更 |
...
└── 使用可能ロール: HannibalCICDRole-Dev
   ...
   ├── アタッチポリシー: HannibalCICDPolicy-Dev-storage（S3・RDS・SecretsManager・KMS）
   ├── アタッチポリシー: HannibalCICDPolicy-Dev-deploy（CloudWatch・CloudFront・Route53・CodeDeploy・SNS・CloudTrail・IAM）
   └── アタッチポリシー: HannibalCICDPolicy-Dev-synthetics（CloudWatch Synthetics・cwsyn Lambda・canary log・X-Ray read・canary実行role PassRole）
...
🧨 fis.amazonaws.com (Game Day演習実行・#446)
└── 使用ロール: HannibalFISRole-Dev
   ├── Permission Boundary: HannibalFISBoundary-Dev（ECSタスク停止のみ、nestjs-hannibal-3-clusterに限定）
   └── アタッチポリシー: HannibalFISPolicy-Dev（同一権限）
...
### 最小権限化の設計方針
- **write/exec**: 必要な action を明示列挙し、Resource も対象リソースに絞る
- **read 系**: `Get*`/`Describe*`/`List*` プレフィックス wildcard を意図的に使用。AWS が API を追加しても policy 変更なしで対応できる。read-only のためリスクは限定的
- **使用していないサービスの権限**: Access Advisor で実測し、呼び出しコードが存在しないことを確認した上で削除（#293）
- **PR plan Role 分離**: CICDRole（deploy/destroy）と PRPlanRole（PR plan）を分離し、PR gate の権限を read-only に限定（#127）
...
| テスト | 結果 | 備考 |
|---|---|---|
| `terraform/environments/dev plan`（lock あり） | AccessDenied | 当時の DynamoDB lock 権限不足。S3 lockfile 移行により解消済み |
...
**現在の状態**:
全 5 root module（foundation/network/database/service/cdn）は `use_lockfile = true` の S3 lockfile 単独で運用し、DynamoDB lock 権限を持たない。
