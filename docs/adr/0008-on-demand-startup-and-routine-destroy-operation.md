...
このプロジェクトは DevOps / SRE / Platform Engineering 向けのポートフォリオであり、採用担当者に見せることを前提にしている。ECS Fargate / RDS / ALB / CodeDeploy Blue-Green といった動的なインフラを、IaC で再現可能な形で示すことに価値がある。

一方で、これらの一時リソースを常時起動しておくと、デモしていない時間帯にも継続的に課金が発生する。ポートフォリオ用途では稼働率が低く、待機している時間がほとんどになる。
