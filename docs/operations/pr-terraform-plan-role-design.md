...
- #122 時点の Terraform root module は `terraform/environments/dev`
- #397 / ADR 0020 後は `terraform/network`、`terraform/database`、`terraform/service`、`terraform/cdn` に state 分割済みであり、#410 で PR plan workflow は一時停止している
- PR plan を再導入する場合は、state 分割後の root module / state key 前提でこの workflow contract を見直す
- #122 時点の backend は S3 bucket `nestjs-hannibal-3-terraform-state`、key `environments/dev/terraform.tfstate`
...
\# 122 の workflow は、このRoleを次の前提で使う設計でした。
\# 410 で `Terraform Plan Change Detection` / `Terraform Plan Artifact` を `pr-check.yml` から一時的に削除したため、同 Issue 時点では PR workflow からこの Role を assume しません。
Role / Boundary / Policy は foundation 側の永続リソースとして保持し、PR plan を再導入するときにこの設計を見直します。
...
- #122 時点では `terraform/environments/dev` で backend ありの `terraform init` を実行する
- 再導入時は state 分割後の root module 構成、または PR plan 専用 composite root module のどちらを使うか再設計する
