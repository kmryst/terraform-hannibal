...
- Commitlint / Sync Labels は `idp-golden-path` の reusable workflow を `@v1` で消費する薄い caller workflow に移行済み（Issue #495 / #498）。PR Policy Check / Gitleaks はこのリポジトリ内のローカル workflow のまま。
