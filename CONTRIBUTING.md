...
ローカル開発ツールの version は `.mise.toml` で管理します。
作業前に次を実行し、Terraform、Node.js、pre-commit、terraform-docs、TFLint を揃えてください。
...
Web UI の Issue Form も、CLI / API / AI Agent からの起票も許容します。
...
AI Agent を使う場合は、ブランチを切った後、実装を始める前に実装計画を提示し、人間が確認してから実装を開始します。
計画には、変更対象ファイル・追加/変更する内容の概要・既存動作への影響を含めます。
...
この節を Conventional Commits / commitlint 運用ルールの正本とします。
...
`header` は100文字以内にしてください。

AI Agent がコミットを作成する場合も、このルールに必ず従います。
`wip`、`fix` のみ、`update files` のような曖昧なコミットメッセージは使いません。

PR作成前に、対象コミットが commitlint を通ることを確認します。
...
git commit -m "ci(commitlint): PR titleとコミットメッセージを検査する"
```
...
PR title も commitlint の対象です。
許可 type と形式は「3. 実装・コミット」の Conventional Commits ルールに従います。

AI Agent を使う場合は、PR もいきなり作成せず、先に PR プランを提示して人間が確認してから作成します。
PR プランには、少なくともタイトル案、`目的`、`変更内容`、`影響範囲`、`Closes/Fixes/Refs #<issue番号>`、推奨ラベルとしての `type/area/risk/cost`、軽運用 / 厳密運用の判定と理由、厳密運用の場合は `ロールバック` が必須かどうか、`使用ヘルパー: ./scripts/github/create-pr-with-labels.sh` を明示して含めてください。
...
ただし、docs のみの変更でも IAM / OIDC / Secrets / Network / Security / deploy / destroy / コスト影響 / ロールバック方針に関わる場合は厳密運用で扱います。
...
厳密運用PRでは、PR本文の `ロールバック` に実質的な内容を書きます。
...
`Terraform Plan Artifact` は **required status check に含まれていません**。

terraform plan はレビュー補助として Job Summary と artifact に出力しますが、plan 結果でマージを止める設計にはしていません。
