...
AI Agent の作業は、実装前の計画提示を運用ルールとし、PR 作成後は Issue link / label / rollback 欄 / CI で機械的に検査する。AI が関与した変更でも、意図・判断・差分・検証結果を追跡できる。AI が日常的に変更を提案・実装する環境で、人間がレビューできる履歴を設計として担保している。
...
- **Issue / PR / Label / CI のガードレール整備**<br>
  整備前はラベル・本文項目が任意で GitHub Flow が統一されていなかった。整備後は 4 カテゴリのラベルを必須化し、PR Policy Check で Issue link・ラベル・rollback 欄を自動検査する状態になった（例: [#92](https://github.com/kmryst/terraform-hannibal/issues/92), [#93](https://github.com/kmryst/terraform-hannibal/pull/93)）
- **PR Terraform plan レビュー基盤の追加**<br>
  整備前は PR での terraform plan に安全な権限境界がなく、apply 権限と共用の role で実行していた。整備後は pull_request context 限定の read-only 専用 role（HannibalPRPlanRole-Dev）を新設し、plan artifact・Job Summary・危険シグナル抽出で apply 前に差分・影響範囲・危険操作をレビューできる状態にした（例: [#121](https://github.com/kmryst/terraform-hannibal/issues/121), [#135](https://github.com/kmryst/terraform-hannibal/pull/135)）
- **AI Agent 作業の履歴品質改善**<br>
  整備前は AI Agent の実装にフォーマルなレビュープロセスがなく、曖昧なコミットメッセージが混在していた。整備後は実装前の計画提示・Conventional Commits 遵守を CONTRIBUTING.md / CLAUDE.md に明文化し、AI 生成コミットでも人間の開発標準に統一した（例: [#152](https://github.com/kmryst/terraform-hannibal/issues/152), [#200](https://github.com/kmryst/terraform-hannibal/issues/200)）
- **IaC security scan 導入と Trivy 検出の棚卸し**<br>
  整備前は IaC の lint と security scan が PR CI に存在せず、tfsec を個別で使用していた。整備後は TFLint / Trivy Config / Gitleaks を PR CI に統合し、検出結果を修正対象・accepted risk・後続検討に分類して Issue 化した（例: [#226](https://github.com/kmryst/terraform-hannibal/issues/226), [#227](https://github.com/kmryst/terraform-hannibal/pull/227)）
- **IAM / OIDC 権限の段階的な最小権限化**<br>
  整備前は deploy / plan が共用の CICD role で実行され、write 権限が混在していた。整備後は deploy / PR plan の role を分離し、permission boundary の二重制御で misconfig 時の blast radius を最小化した（例: [#129](https://github.com/kmryst/terraform-hannibal/issues/129), [#179](https://github.com/kmryst/terraform-hannibal/pull/179)）
