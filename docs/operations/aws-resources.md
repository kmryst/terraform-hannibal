...
| S3 バケット | `nestjs-hannibal-3-cloudtrail-logs` | CloudTrail 監査ログの保存先。`prevent_destroy = true` 設定済み |
...
| CloudWatch Logs Log Group | `/aws/cloudtrail/nestjs-hannibal-3` | CloudTrail management events の即時検知・初動調査用。90日保持 |
