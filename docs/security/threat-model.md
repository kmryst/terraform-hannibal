...
| T10 | supply chain / GitHub Action / Docker image 依存の侵害 | 外部 action、workflow 内で実行する Docker image、依存更新を経由して CI 上で不正コードが実行される | GitHub-owned actions（`actions/*`, `github/codeql-action/*`）は `@vX.Y.Z` semver patch tag で固定し、Dependency graph / Dependabot alerts / Dependabot security updates の対象として維持する。non-GitHub-owned actions は `@<full-length-sha> # vX.Y.Z` で SHA pin し改ざん耐性を確保する（[ADR 0017](../adr/0017-pin-github-actions-by-owner-tier.md)）。workflow 内で `docker run` する外部 Docker image は `image:tag@sha256:<digest>` で固定し、tag なし / `latest` 相当の silent drift を避ける（[ADR 0025](../adr/0025-pin-github-actions-docker-images-by-tag-and-digest.md)）。Dependabot version updates で Tier A の semver tag と Tier B の SHA / コメントを週次追従する。PR gate で build / test / scan を実行する。重い security scan は週次 / 手動 workflow に分離する | non-GitHub-owned actions では SHA pin により Dependabot alerts / security updates が対象外になる。Docker image digest も alerts / security updates の自動追従が弱いため、version updates・release notes 確認・GitHub Advisory Database / upstream advisory 確認・SHA/tag/digest 対応確認で補完する。2025 年の tj-actions/changed-files 改ざん事案を受け、T10 エスカレーション条件を発動し SHA pin を採用した。Dependency graph / Dependabot alerts / security updates の有効化状態、現在 pin している action の advisory 確認結果（2026-03 の `aquasecurity/trivy-action` tag 改ざん事案 GHSA-69fq-xp46-6x23 を含む）は [action-pin-review.md](../operations/action-pin-review.md) に記録する |
