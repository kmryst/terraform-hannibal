...
このリポジトリの主要ツールは `.mise.toml` で管理します。
...
`mise install` は Terraform、Node.js、pre-commit、terraform-docs、TFLint のバージョンを揃えます。
Terraform root module の README は terraform-docs により pre-commit で更新します。
