...
### DynamoDB テーブル（レガシー — 削除予定）

| 項目 | 値 |
|---|---|
| リソース | Amazon DynamoDB |
| テーブル名 | terraform-state-lock |
| 用途 | 旧 DynamoDB-based locking 用。全 root module が S3 lockfile に移行済み |
| Terraform 参照方法 | 現在どの backend.tf からも参照していない（foundation のみ移行期間中に併用していたが、他 module は S3 lockfile 単独） |

**手動管理の理由**

全 root module が `use_lockfile = true` に移行済み。テーブル本体の削除は #189 で扱う。
