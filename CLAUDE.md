...
補足: `gh pr view` / `gh pr edit` が GitHub CLI の GraphQL `projectCards` deprecated 警告で失敗しても、PR 作成自体が失敗したとは限らない。PR の本文・ラベル・状態確認や本文更新は、必要に応じて `gh api repos/<owner>/<repo>/pulls/<PR番号>` などの REST API で確認・更新する。
